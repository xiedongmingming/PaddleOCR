...
class PicoDetPostProcess(object):
    ...
    def __init__(
        ...
        self.labels = self.load_layout_dict(layout_dict_path)  # ['text', 'title', 'figure', 'figure_caption', 'table', 'table_caption', 'header', 'footer', 'reference', 'equation']

        self.strides = strides  # ？？？

        self.score_threshold = score_threshold  # 0.5

        self.nms_threshold = nms_threshold  # 0.5

        self.nms_top_k = nms_top_k  # 1000

        self.keep_top_k = keep_top_k  # 100
    ...
    def img_info(self, ori_img, img):  # ori_img: ndarray(1684, 1191, 3) img: ndarray(1, 3, 800, 608)

        origin_shape = ori_img.shape  # (1684, 1191, 3)

        resize_shape = img.shape  # (1, 3, 800, 608)

        im_scale_y = resize_shape[2] / float(origin_shape[0])  # 800/1684
        im_scale_x = resize_shape[3] / float(origin_shape[1])  # 608/1191

        scale_factor = np.array([im_scale_y, im_scale_x], dtype=np.float32)  # [0.4750594  0.51049536]
